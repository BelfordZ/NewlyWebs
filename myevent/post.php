...
case 'edit':
	...
	if ( empty( $post_id ) ) {
		...
		wp_redirect( site_url('myevent/post.php') );
		exit();
	}
	...
	include('./myevent/edit-form-advanced.php');
...
case 'editpost':
	...
	// Check if this page exists in navigation menu
	$found = false;
	$menu_item_count = 0;
	...
	if (!$found) {
		...
		$itemData = array(
		    ...
		    'menu-item-position'  => ($menu_item_count+1), // add as a last menu item
	...
	$post_page_template = get_post_meta($post_id, '_wp_page_template', yes);
	...
	if (function_exists('update_page_content'))
		update_page_content($_POST);

	//redirect_post($post_id); // Send user on their way while we keep working
	wp_redirect( site_url('myevent/post.php?post='.$post_id.'&action=edit') );
...
case 'delete':
	...
	// find menu_item_id using post_id
	$menu_item_id = 0;
	$menu = wp_get_nav_menu_object('menu');
	$menu_items = wp_get_nav_menu_items($menu->term_id);
	foreach ($menu_items as $menu_item) {
...
default:
	...
	wp_redirect( site_url('myevent/edit1.php') );
	exit();
	break;
} // end switch
